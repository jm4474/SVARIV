...
% 2.4 GHz Intel Core i7 with 8 GB in Memory (OSX El Capitan)
...
RForm=load('../5MCparameters/RForm_Kilian.mat');
...
B1= MC.Gamma./((MC.Gamma'*(MC.Sigma^-1)*MC.Gamma)^.5); %as described in our paper
...
MC.n=size(MC.AL,1);
Z=RForm.externalIV;
[MC.WHatall,MC.WHat,MC.V] = CovAhat_Sigmahat_Gamma(RForm.p,RForm.X,Z,RForm.eta); 
MC.alpha=((MC.Gamma'*(MC.Sigma^-1)*MC.Gamma)^.5);
MC.impliedfirststage=size(RForm.eta,2)*((MC.alpha*MC.B(1,1))^2)./MC.WHat(((MC.n^2)*(RForm.p))+1,((MC.n^2)*(RForm.p))+1);
MC.muZ=mean(Z);
MC.varZ=var(Z);
clear Z;
MC.sigmav=(MC.varZ-(MC.alpha.^2))^.5;
MC.p=RForm.p; 
MC.Y0=RForm.Y0; 
%MC.T=size(RForm.Y,1);
MC.T=800;
MC.hori=hori; 
MC.x=x; clear RForm hori x
...
MC.IRFZ(:,:,1)=reshape(sum(bsxfun(@times,C,B1'),2),[MC.n,MC.hori+1]);
MC.IRFZ(:,:,2)=reshape(sum(bsxfun(@times,Ccum,B1'),2),[MC.n,MC.hori+1]);
...
MC.B1Chol=MC.Sigma(:,1)./(MC.Sigma(1,1)^.5);
B1Chol=MC.x*MC.B1Chol./MC.B1Chol(1,1);

MC.IRFChol(:,:,1)=reshape(sum(bsxfun(@times,C,B1Chol'),2),[MC.n,MC.hori+1]);
MC.IRFChol(:,:,2)=reshape(sum(bsxfun(@times,Ccum,B1Chol'),2),[MC.n,MC.hori+1]);
...
coverageMCMSW=zeros(MC.n,MC.hori+1,MCdraws);
coverageMCdmethod=zeros(MC.n,MC.hori+1,MCdraws);
IRFMC.IRFplugin=zeros(MC.n,MC.hori+1,MCdraws);
IRFMC.dmethodstderr=zeros(MC.n,MC.hori+1,MCdraws);

for mcdraw=1:MCdraws;   
lagsDATAMC=zeros(MC.n,MC.p,1+MC.T+burnout);
lagsDATAMC(:,:,1)=fliplr(MC.Y0'); %Y_{t-1},Y_{t-2}, ... Y_{t-p}
DATAMCaux=zeros(MC.n,MC.T+burnout);  %Initialize the data
EIVMCaux=zeros(1,MC.T+burnout);      %Initialize the instrument
residuals=randn(1+MC.n,MC.T+burnout);%Structural Innovations 
for ix=1:MC.T+burnout;
    rformresiduals=((MC.B))*residuals(1:MC.n,ix);
    DATAMCaux(1:MC.n,ix)=(MC.mu)+(MC.AL*reshape(lagsDATAMC(:,:,ix),[MC.n*MC.p,1]))...
        +(rformresiduals);
    lagsDATAMC(:,1,ix+1)=DATAMCaux(:,ix);
    lagsDATAMC(:,2:MC.p,ix+1)=lagsDATAMC(:,1:MC.p-1,ix);
    EIVMCaux(1,ix)=MC.muZ+(MC.alpha*residuals(1,ix))+(MC.sigmav*residuals(MC.n+1,ix));
...
clearvars -except MC MCdata MCdraws mcdraw coverageMCMSW coverageMCdmethod burnout IRFMC
...
[RFormMC.WHatall,RFormMC.WHat,RFormMC.V] = CovAhat_Sigmahat_Gamma(MC.p,RFormMC.X,MCdata.Z(MC.p+1:end,1),RFormMC.eta);                
...
clearvars -except MC coverageMCMSW coverageMCdmethod MCdraws mcdraw InferenceMSWMC burnout IRFMC
...
ylabel('MC Coverage');
title(strcat('Cumulative Response of Oil Production (',num2str(MCdraws),'MC draws, T=',num2str(InferenceMSWMC.T),', alpha=',num2str(round(MC.alpha,2)),')')); 
...
%%Note that the sign of N is reversed: ¡°round(X,N)¡± should be replaced with
%%¡°roundn(X,-N)¡±.
...
title(strcat('Response of Global Real Activity (',num2str(MCdraws),'MC draws, T=',num2str(InferenceMSWMC.T),', alpha=',num2str(round(MC.alpha,2)),')')); 
...
xlabel('Months after the shock');
ylabel('MC Coverage');
title(strcat('Response of the Real Price of Oil (',num2str(MCdraws),'MC draws, T=',num2str(InferenceMSWMC.T),', alpha=',num2str(round(MC.alpha,2)),')')); 
...
cd('../6SVAR_MSW_pics');
namdir=strcat(date,'_','K','_','p=',num2str(MC.p),'confidence0.95','_Design1');
if exist(namdir,'dir')
    cd(namdir);
    print(figure(1),'-depsc2',strcat('MC_Coverage_KilianDGP','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_alpha=',num2str(round(MC.alpha,2)),'.eps'));
    print(figure(2),'-depsc2',strcat('MC_IRFDist','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_alpha=',num2str(round(MC.alpha,2)),'.eps'));
    save(strcat('MC_Coverage_KilianDGP','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_alpha=',num2str(round(MC.alpha,2)),'.mat'));
    cd ..
    cd ..
else    
    mkdir(namdir);
