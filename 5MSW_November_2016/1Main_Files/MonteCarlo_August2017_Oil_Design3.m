...
[MC.WHatall,...
 MC.WHat,...
 MC.V]   = CovAhat_Sigmahat_Gamma(RForm.p,RForm.X,Z,RForm.eta); 
...
MC.alpha = (.1./ (MC.B(1,1)))*((MC.varZ.*MC.sigma2Gamma1./MC.T).^.5);

MC.impliedfirststage ...
         = MC.T*((MC.alpha*MC.B(1,1))^2)./(MC.varZ.*MC.sigma2Gamma1);
...
for ix=1:MC.T+burnout
    ...
    EIVMCaux(1,ix)...
        = MC.muZ+(MC.alpha*residuals(1,ix))+(MC.sigmav*residuals(MC.n+1,ix));
...
if exist(namdir,'dir')
    cd(namdir);
    print(figure(1),'-depsc2',strcat('MC_Coverage_KilianDGP','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_CP=',num2str(round(MC.impliedfirststage,2)),'.eps'));
    print(figure(2),'-depsc2',strcat('MC_IRFDist','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_CP=',num2str(round(MC.impliedfirststage,2)),'.eps'));
    save(strcat('MC_Coverage_KilianDGP','_p=',num2str(MC.p),'_T=',num2str(InferenceMSWMC.T),'_CP=',num2str(round(MC.impliedfirststage,2)),'.mat'));
    cd ..
    cd ..
else    
    mkdir(namdir);
