...
plots.name(1,:) = {'Log(1/1-AMTR) Bottom 99%'};
 
plots.name(2,:) = {'Log Income Bottom 99%'};
 
plots.name(3,:) = {'Log Real GDP'};
 
plots.name(4,:) = {'Unemployment Rate'};

plots.name(5,:) = {'Log(1/1-AMTR) Top 1%'};

plots.name(6,:) = {'Log Income Top 1%'};
...
for iplot = 1:6
    ...
    plot(0:1:hori,Plugin.IRF(plots.index(1,iplot),:),'b'); hold on
    
    [~,~] = jbfill(0:1:hori,InferenceMSW.MSWubound(plots.index(1,iplot),:),...
...
output_label = strcat('_p=',num2str(p),'_Bottom99Top1_',...
