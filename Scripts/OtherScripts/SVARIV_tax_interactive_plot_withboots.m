...
clear ctype ok1 ok2 prompt1 prompt2 strs1 strs2
...
%% 7) "Standard" bootstrap-type inference based on samples from the asy dist.
...
%% 8) Comparison of "standard" bootstrap inference and the delta-method
...
for iplot = 1:4
    ...
    g1    =  plot(0:1:hori,InferenceMSW.bootsIRFs(iplot,:,2),':b'); hold on
    ...
    g2    =  plot(0:1:hori,InferenceMSW.bootsIRFs(iplot,:,1),':b'); hold on
...
%% 9) Save the output and plots in ./Output/Mat and ./Output/Figs
 
disp('-')
 
disp('9) This section saves the .mat files and figures in the Output folder')
...
output_label = strcat('_p=',num2str(p),'_',char(labelstrs(1,dtype)),'_',...
...
print(gcf,'-depsc2',strcat('IRF_SVARstdboots',output_label,'.eps'));
