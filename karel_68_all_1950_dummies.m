...
disp('This script reports 68% confidence bands for IRFs using AMTR and Income for all units ')

%a) All units
[data.years,~,~]=xlsread('DATA_Mertens2015','AMTR (Figure 1)','A6:A68');
[data.Var1_AMTR,~,~]=xlsread('DATA_Mertens2015','AMTR (Figure 1)','C6:C68');
[data.Var2_LogIncome,~,~]=xlsread('DATA_Mertens2015','LOG AVG INCOME','B6:B68');
[data.Var3_Controls,data.Var3_Labels,~]=xlsread('DATA_Mertens2015','CONTROLS','B6:H68');
[data.Var4_ExtIV,~,~]=xlsread('DATA_Mertens2015','PROXIES (Table 3)','C6:C68');
...
d2=(data.years(p+1:end,:)==2008); %dummy2008

Waux=double(d2); clear d2
W=eye(size(data.years(p+1:end,:),1))...
    -(Waux*((Waux'*Waux)^(-1))*Waux'); clear Waux
...
%a) Estimation of (AL, Sigma) and the reduced-form innovations
    ...
    [RForm.mu,RForm.AL,RForm.Sigma,RForm.eta,RForm.X,RForm.Y] = RForm_VAR(SVARinp.ydata,p,W);
...
print(gcf,'-depsc2','karel_68_all_1950_dummies.eps');


%% 


save('karel_68_all_1950_dummies.mat');
